...
\begin{description}
    \item[Contradiction]
        ...
        Since $n$ is finite, an optimal solution $J = j_1, \cdots, j_m$ exists (meaning $m > k$), that matches our algorithm's greedy solution for the largest possible contiguous chunk of indices from the beginning. Let $r$ be the last index, where $i_1 = j_i, i_2 = j_2, \cdots, i_r = j_r$ (default 0).
    ...
    \item[Induction] The induction case uses a very similar argument, which argues that every partial solution from our algorithm is a subset of a optimal solution, where optimality once again stems from our algorithm's choice of the next compatible text with the lowest finishing time.
