...
Another category is \textit{sequential circuits} which we will learn about later, which rely on memory. De-multiplexers may be testable, you should implement one in your own time.
...
Suppose we are given some signed $b$-bit integer, $n$. Notice that when $n$ is summed with its one's complement, we would get exactly $2^b - 1$. Adding one again, we take advantage of the overflow, and get zero, as we wanted. Note that for a $b$-bit integer to be positive, its signed bit (the most significant bit) must be zero. It's two's complement would have a sign bit of 1, indicating that it is indeed a negative number.
