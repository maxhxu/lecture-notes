...
\]
where $Q$ is the set of states, $\Sigma$ is the set of `inputs' to the circuit, $\delta : Q \times \Sigma \to Q$ is the transition function, $q_0$ is the starting state, and $F$ are the accepting states. 

We suppose that exactly \textbf{one} transition occurs per clock tick. The slides say that you can have multiple starting states, but this is just for convenience. Rigorously, you would just have a dummy starting state $q_0$, which then transitions to one of your `actual' starting states after one tick.
