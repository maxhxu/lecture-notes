...
An analogy is that the probability of you having to wait an additional $b$ minutes, is `independent' of you having already waited $a$ minutes. 
...
Suppose we have $n$ houses in a city, with each house independently having a $\theta$ probability of a fire on. The number of houses on fire $\lambda = n \theta$. We can compute the probability of having $k$ houses on fire as follows:
\[
P(\text{\# fires} = k) = \binom{n}{k} (1-\theta)^{n-k} \theta^{k}
\]

We will be skipping the Negative-Binomial($r, \theta$) and Hypergeometric($N$, $M$, $n$) distributions, and cover them next year.
...
This follows almost immediately from \ref{thm:ltpcond}, since $X = x_i$ for some $i$ will always occur, by definition of $x_i$, thus they union to $S$. For pairwise disjointedness $X$ can only take on one value at a time, hence this is a valid partition.
