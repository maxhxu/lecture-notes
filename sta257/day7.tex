...
We write $P(X \in B)$ to mean $P(X^{-1}(B)) := P(\{ s \in S: X(s) \in B \})$. We call $X^{-1}(B)$ the inverse image of $B$. We can also write $P(X = x) = P(X^{-1}(\{ x \})) = P(\{s \in S : X(s) = x\})$
...
There exists a special case $\theta = \frac{1}{2}$, making $P(X = k) = \frac{\binom{n}{k}}{2^{n}}$. Bernoulli($\theta$) is the same as Binomial(1, $\theta$). Suppose $X_1, \cdots, X_n \sim \text{Bernoulli}(\theta)$ are independent random variables. We claim that the distribution of $Y = \sum_i X_i \sim$, $Y \sim \text{Bernoulli}(n, \theta)$.

Picking independently $k$ elements at random with replacement, with binary outcomes is usually $P(X = k)$, where $X \sim \text{Binomial}(n, \theta)$? 
