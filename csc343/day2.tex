...
Any superset of a key is called a \textbf{superkey} (useful in
theory). We mostly declare keys in practice. Immediately, every key
is a superkey.

\begin{definition}[Foreign Key]
    A foreign key is a subset of attributes of some relation $R_1$,
    such that it is also a key for relation $R_2$.
\end{definition}

Let $R$ be a relation, and $A$ a list of attributes in $R$. Define
$R[A]$ as the set of all attributes from $R$, with only the
attributes in list $A$.

\textbf{Foreign key constraints} are declared as $R_1[X] \subseteq
R_2[Y]$, where $X$, $Y$ have the same arity, and $Y$ must be a key in
$R_2$. This is an instance of a \textbf{referential integrity
constraint} (though not all referential integrity constraints are
