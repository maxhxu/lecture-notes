\section{Tutorial 5 (Oct 10, 2025)}
...
\begin{problem}
\end{problem}
\includegraphics{csc265/figures/tut5.jpg}
...
\begin{description}
    ...
    Want to show that our credit invariant holds, that each non-empty entry in the second half of $A$ has \$2, as well as this inequality, where $\epsilon_i$ is the amortized cost, and $c_i$ is the actual cost. 
    ...
    \begin{align*}
        \sum_{i=1}^{m+1} \epsilon_i &= \sum_{i=1}^{m} \epsilon_i + 3 \\
        &\geq \sum_{i=1}^{m} c_i + (\text{old sum of credits}) + 3 \\
        &= \sum_{i=1}^{m} c_i + (\text{new sum of credits} - 2) + 3 \\
        &= \sum_{i=1}^{m+1} c_i
    \end{align*}

    \textit{Case 2}: Suppose that $A$ is full (i.e. $\ell = m = n + 1$ at the start of the $(m+1)$-th operation). Copy the elements of $A$ into the first half of $A'$, which costs \$$n$ . We pay for the transfer using the $\frac{n}{2} \cdot \$2$ of credits in the second half of $A$. This amount of credits exists by the invariant, since we know that $A$ is full. 
    ...
    \begin{align*}
        \sum_{i=1}^{m+1} \epsilon_i &= \sum_{i=1}^{m} \epsilon_i + 3 \\
        &\geq \sum_{i=1}^{m} c_i + \underset{\geq n}{(\text{old credits in DS})} + 3 \\
        &= \sum_{i=1}^{m} c_i + (\text{new credits in DS}) + n - 2 + 3 \\
        &= \sum_{i=1}^{m} c_i + n + 1 + (\text{new credits in DS}) \\
        &= \sum_{i=1}^{m+1} c_i + (\text{new credits in DS})
    \end{align*}

    \item[Step 5] State the conclusion. \\
    By the previous inequality, the sum of the amortized costs ($\epsilon_i$) is at least the sum of the actual costs ($c_i$) of the $m$ operations. Since each \textsc{AddToEnd} operation has amortized cost \$3, the (actual) cost for performing a sequence of $m$ operations is upper bounded by $3m$. 
